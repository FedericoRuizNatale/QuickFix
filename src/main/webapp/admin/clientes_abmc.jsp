...
<%-- Seguridad: Verificar Sesión y Rol de Admin --%>
...
<head>
    <meta charset="UTF-8">
    <title>ABMC Clientes - Admin</title>
...
<body>
    ...
    <%-- Incluir la Barra de Navegación del Admin --%>
    ...
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mb-3">
            <li class="nav-item">
                <a class="nav-link active" href="AdminUsuariosServlet?rol=cliente">Clientes</a>
            </li>
            <li class="nav-item">
        ...
        <c:if test="${param.exito == 'creado'}"> <div class="alert alert-success">¡Cliente creado!</div> </c:if>
        <c:if test="${param.exito == 'editado'}"> <div class="alert alert-success">¡Cliente actualizado!</div> </c:if>
        <c:if test="${param.exito == 'eliminado'}"> <div class="alert alert-success">¡Cliente eliminado!</div> </c:if>
            ...
            <div class="card-body">
                <table class="table table-hover table-striped align-middle">
                    <thead class="table-dark">
                        <tr>
                            <th>ID</th> <th>Nombre</th> <th>Email</th> <th>DNI</th> <th>Estado</th> <th class="text-center">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        <c:forEach var="user" items="${listaUsuarios}">
                            <tr>
                                ...
                                <td>${user.email}</td>
                                <td>${user.dni}</td> <%-- Campo específico --%>
                                <td>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-warning me-1" 
                                            onclick="abrirModalEdicion(
                                                ...
                                                '${user.telefono}', '${user.direccion}', ${user.estado}, '${user.dni}'
                                            )">
                                        <i class="fas fa-edit"></i> Editar
                                    </button>
                                    <form action="AdminUsuariosServlet" method="POST" style="display:inline;" onsubmit="return confirm('¿Seguro? Se eliminará el usuario.');">
        ...
        <div class="modal-content">
          ...
          <form id="formUsuario" action="AdminUsuariosServlet" method="POST"> 
             <div class="modal-body">
                ...
                <input type="hidden" name="rol" value="cliente"> <%-- Rol fijo para este JSP --%>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3"><label>DNI:</label><input type="text" class="form-control" id="dni" name="dni" required></div>
    ...
    <script>
        ...
        function abrirModalCreacion() {
            // Resetear todo para "Crear"
            modalLabel.textContent = 'Crear Nuevo Cliente';
            btnGuardar.textContent = 'Guardar';
            btnGuardar.classList.remove('btn-warning');
            btnGuardar.classList.add('btn-primary');
            
            formUsuario.reset(); // Limpia todos los campos
            document.getElementById('formAction').value = 'create';
            document.getElementById('formIdUsuario').value = '0';
            document.getElementById('estado').checked = true; // Activo por defecto
            document.getElementById('password').required = true; // Contraseña obligatoria al crear

            modalUsuario.show();
        }

        function abrirModalEdicion(id, nombre, apellido, email, telefono, direccion, estado, dni) {
            // Configurar para "Editar"
            modalLabel.textContent = 'Editar Cliente (ID: ' + id + ')';
            btnGuardar.textContent = 'Guardar Cambios';
            btnGuardar.classList.remove('btn-primary');
            btnGuardar.classList.add('btn-warning');
            
            formUsuario.reset(); // Limpia por si acaso
