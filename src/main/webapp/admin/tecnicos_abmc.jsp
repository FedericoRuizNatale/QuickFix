...
<head>
    <meta charset="UTF-8">
    <title>ABMC Técnicos - Admin</title>
...
<body>
    ...
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mb-3">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="AdminUsuariosServlet?rol=tecnico">Técnicos</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="AdminUsuariosServlet?rol=administrador">Administradores</a>
            </li>
        </ul>

        <%-- Botón CREAR --%>
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3 class="h4">Listado de Técnicos</h3>
            <button type="button" class="btn btn-primary" onclick="abrirModalCreacion()">
                <i class="fas fa-plus me-2"></i> Crear Nuevo Técnico
            </button>
        </div>
        
        <%-- Mensajes --%>
        <c:if test="${param.exito == 'creado'}"> <div class="alert alert-success">¡Técnico creado!</div> </c:if>
        <c:if test="${param.exito == 'editado'}"> <div class="alert alert-success">¡Técnico actualizado!</div> </c:if>
        <c:if test="${param.exito == 'eliminado'}"> <div class="alert alert-success">¡Técnico eliminado!</div> </c:if>
            ...
            <div class="card-body">
                <table class="table table-hover table-striped align-middle">
                    <thead class="table-dark">
                        <tr>
                            <th>ID</th> <th>Nombre</th> <th>Email</th> <th>Especialidad</th> <th>Disponibilidad</th> <th>Estado</th> <th class="text-center">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        <c:forEach var="user" items="${listaUsuarios}">
                            <tr>
                                ...
                                <td>${user.email}</td>
                                <td>${user.especialidad}</td> <%-- Específico --%>
                                <td>${user.disponibilidad}</td> <%-- Específico --%>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-warning me-1" 
                                            onclick="abrirModalEdicion(
                                                ...
                                                '${user.telefono}', '${user.direccion}', ${user.estado}, 
                                                '${user.especialidad}', '${user.disponibilidad}'
                                            )">
                                        <i class="fas fa-edit"></i> Editar
                                    </button>
                                    <form action="AdminUsuariosServlet" method="POST" style="display:inline;" onsubmit="return confirm('¿Seguro? Se eliminará el técnico.');">
        ...
        <div class="modal-content">
          ...
          <form id="formUsuario" action="AdminUsuariosServlet" method="POST"> 
             <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3"><label>Especialidad:</label><input type="text" class="form-control" id="especialidad" name="especialidad" required></div>
                    <div class="col-md-6 mb-3"><label>Disponibilidad (Texto):</label><input type="text" class="form-control" id="disponibilidad" name="disponibilidad" placeholder="Ej: L-V 9-18hs"></div>
                </div>
                 <div class="row"><div class="col-md-6 mb-3"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="estado" name="estado" checked><label class="form-check-label ms-2" for="estado">Usuario Activo</label></div></div></div>
             </div>
    ...
    <script>
        ...
        function abrirModalCreacion() {
            modalLabel.textContent = 'Crear Nuevo Técnico';
        ...
        function abrirModalEdicion(id, nombre, apellido, email, telefono, direccion, estado, especialidad, disponibilidad) {
            modalLabel.textContent = 'Editar Técnico (ID: ' + id + ')';
