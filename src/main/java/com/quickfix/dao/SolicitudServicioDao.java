...
public class SolicitudServicioDao extends GenericDao<SolicitudServicio, Integer> {
	...
	public List<SolicitudServicio> findByCliente(Cliente cliente) {
		...
		try {
			TypedQuery<SolicitudServicio> query = em.createQuery(
					...
					"SELECT s FROM SolicitudServicio s WHERE s.cliente = :cliente ORDER BY s.fechaHoraCreacion DESC", // Ordena
																														// por
																														// fecha
		...
		try {
			...
			TypedQuery<SolicitudServicio> query = em.createQuery(
					"SELECT s FROM SolicitudServicio s WHERE s.tecnico = :tecnico "
							+ "AND s.estado NOT IN (:estadosFinalizados) ORDER BY s.fechaHoraCreacion DESC", // Ordenadas
																												// por
																												// fecha
																												// más
																												// reciente
			...
			query.setParameter("estadosFinalizados", List.of(EstadoSolicitud.FINALIZADA, EstadoSolicitud.CANCELADA));
	...
	public List<SolicitudServicio> findSolicitudesSinAsignar() {
	    EntityManager em = getEntityManager();
	    try {
	        TypedQuery<SolicitudServicio> query = em.createQuery(
	                "SELECT s FROM SolicitudServicio s WHERE s.tecnico IS NULL AND s.estado = :estado",
	                SolicitudServicio.class);
	        query.setParameter("estado", EstadoSolicitud.RECIBIDA);
	        
	        List<SolicitudServicio> solicitudes = query.getResultList();

	        // "Despertamos" los datos de forma SEGURA
	        for (SolicitudServicio s : solicitudes) {
	            
	            // Si hay un servicio, "tócalo"
	            if (s.getServicio() != null) {
	                s.getServicio().getNombre();
	            }
	            
	            // Si hay un cliente, "tócalo"
	            if (s.getCliente() != null) {
	                s.getCliente().getNombre();
	            }
	            
	            // Si hay un equipo, "tócalo"
	            if (s.getEquipoCliente() != null) {
	                s.getEquipoCliente().getMarca();
	            }
	        }
	...
	public long countActiveSolicitudesByTecnico(Tecnico tecnico) {
	    EntityManager em = getEntityManager();
	    try {
	        TypedQuery<Long> query = em.createQuery(
	            "SELECT COUNT(s) FROM SolicitudServicio s WHERE s.tecnico = :tecnico " +
	            "AND s.estado NOT IN (:estadosFinalizados)", // Excluye las terminadas
	            Long.class
	        );
	        query.setParameter("tecnico", tecnico);
	        // Define qué estados NO cuentan como carga (ej: FINALIZADA, CANCELADA)
	        query.setParameter("estadosFinalizados", 
	            List.of(EstadoSolicitud.FINALIZADA, EstadoSolicitud.CANCELADA, EstadoSolicitud.LISTA_PARA_RETIRO));
